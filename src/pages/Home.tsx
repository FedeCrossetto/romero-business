...
export function Home() {
  ...
  const { reviews, rating, totalRatings, loading: reviewsLoading } = useGoogleReviews()
          ...
          <AnimateOnScroll>
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-1">
                  ...
                  <span className="text-sm text-gray-500 dark:text-gray-400 ml-1">
                    {rating?.toFixed(1)} en Google{totalRatings ? ` · ${totalRatings} reseñas` : ''}
                  </span>
                </div>
              </div>
              <a
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {reviews.slice(0, 3).map((r) => (
                <div key={r.author_name} className="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-card flex flex-col gap-3">
                  <div className="flex items-center gap-3">
                    {r.profile_photo_url ? (
                      <img src={r.profile_photo_url} alt={r.author_name} referrerPolicy="no-referrer" className="w-10 h-10 rounded-full object-cover shrink-0" />
                    ) : (
                    ...
                    <div>
                      <p className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{r.author_name}</p>
